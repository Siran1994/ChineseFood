...
public class EatFortuneCookieScene : MonoBehaviour {
	...
	public Animator PopupFortuneMessage;

	public Animator animButtonNext;
	public  GameObject ButtonReplay;
	public Canvas canvas;
	public RectTransform eatFortuneCookieImageRT;

	public RawImage eatFortuneCookieImage;
	...
	public Image imageFortuneMessage;
	public Text txtFortuneMessage;
	...
	void CalculatePixelsRect()
	{
		...
		screenScale =    eatTexSize / (float) (eatFortuneCookieImageRT.rect.width * scale);

		pL = Mathf.CeilToInt ( (eatFortuneCookieImageRT.rect.x * scale + eatFortuneCookieImageRT.anchoredPosition.x   * scale  +  Camera.main.pixelWidth/2f) * screenScale) ;
		pR = Mathf.FloorToInt ( ((eatFortuneCookieImageRT.rect.x + eatFortuneCookieImageRT.rect.width)* scale + eatFortuneCookieImageRT.anchoredPosition.x  * scale  + Camera.main.pixelWidth/2f) * screenScale);
		pB = Mathf.CeilToInt ( (eatFortuneCookieImageRT.rect.y * scale + eatFortuneCookieImageRT.anchoredPosition.y   * scale    +   Camera.main.pixelHeight/2f) * screenScale);
		pT = Mathf.FloorToInt (( (eatFortuneCookieImageRT.rect.y + eatFortuneCookieImageRT.rect.height)* scale + eatFortuneCookieImageRT.anchoredPosition.y  * scale  + Camera.main.pixelHeight/2f) * screenScale);
	}
	...
	IEnumerator CreateTexture()
	{
		...
		int texSizeY = Mathf.FloorToInt( eatTexSize* eatFortuneCookieImage.texture.height/eatFortuneCookieImage.texture.width); 
		...
		AlphaScale( (Texture2D)eatFortuneCookieImage.texture, texCopy.width,texCopy.height );
		...
		eatFortuneCookieImage.material.SetTexture("_AlphaTex", texCopy);
	}
	...
	void Update()
	{
		...
		if(  !bShowMessage && bEnableEat && !GlobalVariables.bPauseGame &&    Input.GetMouseButtonDown(0))
		{
	...
	{
      //  SDKManager.Instance.ShowAd(ShowAdType.VideoAD, 2, "重吃幸运饼干");
	...
	IEnumerator LevelCompleted()
	{
        ...
        eatFortuneCookieImage.enabled = false;
		...
		{
			case 0: txtFortuneMessage.text = "A dream you have will come true!"; break;
			case 1: txtFortuneMessage.text = "Your fortune is as sweet as a cookie!"; break;
			case 2:txtFortuneMessage.text = "You will have unexpected good luck."; break;
			case 3:txtFortuneMessage.text = ""; imageFortuneMessage.sprite = GameData.FortuneMessageSprite; break;
			case 4:txtFortuneMessage.text = ""; imageFortuneMessage.sprite = GameData.FortuneMessageSprite; break;
		}
		...
		animTimerAndReplayHolder.SetBool("bShow",false);
		yield return new WaitForSeconds(1f);
		PopupFortuneMessage.gameObject.SetActive(true);
		PopupFortuneMessage.Play("Show");
	...
	{
		Debug.Log("FinishReadingMessage2");

		PopupFortuneMessage.Play("Hide");
	...
	{
        SDKManager.Instance.ShowAd(ShowAdType.ChaPing, 1, "吃完幸运饼干界面完成后点下一步");
