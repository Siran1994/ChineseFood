...
public class RawDoughCut : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler 
{
	...
	IEnumerator Start()
	{
		...
		DragScale = new Vector3(1.6f,1f,1)  ;
		EndScale = new Vector3(1.2f,.75f,1);//Vector3(0.8f,0.5f,1);
	...
	public void SetFryerStartPostionAndScale()
	{
		...
		EndScale = new Vector3(1f,.85f,1) * scaleFactor;
		StartPosition  = transform.position;
	}
	...
	{ 
		if( bEnabled &&  bDrag )
		{
			...
			if(TopMovementLimit ==null ) TopMovementLimit = GameObject.Find("DoughMovementLimit").transform;
	...
	IEnumerator SnapToTarget( Transform target)
	{
		...
		yield return new WaitForSeconds(0.1f);
		 Camera.main.SendMessage("NextPhase","Dough", SendMessageOptions.DontRequireReceiver);
